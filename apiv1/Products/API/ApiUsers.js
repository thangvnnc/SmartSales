...
router.post("/register", async (req, res) => {
    ...
    try {
        let data = await DBUsers.findOne().where(CUser.fds().USERNAME).equals(cUser.username).exec();
        if (data !== null){
...
router.post("/login", async(req, res) => {
    ...
    try {
        ...
        let data = await DBUsers.findOne()
            .where(CUser.fds().USERNAME).equals(cUser.username)
            .where(CUser.fds().PASSWORD).equals(cUser.password);
        if (data !== null){
